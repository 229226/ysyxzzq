...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm;R(rd) = s->snpc;
    #ifdef CONFIG_ITRACE
      ftrace_pcall(s->pc,s->dnpc)
    #endif
  );
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) & (-2); R(rd) = s->snpc;
    #ifdef CONFIG_ITRACE  
      if(s->isa.inst == RV32RET) ftrace_pret(s->pc);
      else ftrace_pcall(s->pc,s->dnpc)
    #endif
  );
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 1), 8));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = (unsigned)src1 << (unsigned)(imm&0b11111));
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (unsigned)src1 >> (unsigned)(imm&0b11111));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (signed)src1 >> (signed)(imm&0b11111));

  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(rd) = src1 - src2);
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = (unsigned)src1 << (src2&0b11111) );
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(rd) = (unsigned)src1 >> (unsigned)(src2&0b11111));
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (signed)src1 >> (signed)(src2&0b11111));
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, uint64_t res = ((uint64_t)src1 * (uint64_t)src2) >> 32;R(rd) = (int32_t)res);
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, if(src2 == 0){R(rd) = -1;}else if((signed)src1 == INT32_MIN && (signed)src2 == -1){R(rd) = INT32_MIN;}else{R(rd) = (signed)src1 / (signed)src2;});
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, if(src2 == 0){R(rd) = -1;}else{R(rd) = (unsigned)src1 / (unsigned)src2;});
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, if(src2 == 0){R(rd) = src1;}else if((signed)src1 == INT32_MIN && (signed)src2 == -1){R(rd) = 0;}else{R(rd) = (signed)src1 % (signed)src2;});
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, if(src2 == 0){R(rd) = src1;}else{R(rd) = (unsigned)src1 % (unsigned)src2;});
